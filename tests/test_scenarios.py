...
def _patch_spine(monkeypatch, *, risk=None):
    ...
    # graph_mod.policy_node stays the REAL Member B detector (offline path, forced
    # by _force_offline above). Also pin retrieval to the local backend so a CI env
    # that DOES carry Supabase/OpenAI secrets never reaches the network here (the
    # patched get_embeddings would otherwise raise inside Policy's retrieval).
